...
public class EnderecoSeguradoDao {
	...
	public List<EnderecoSegurado> pesquisarPorBairro(String bairro) throws SQLException{
		...
		PreparedStatement stm = conn.prepareStatement("select * from t_sip_endereco_segurado where id_endereco_segurado like ?");
		
		stm.setString(1, "%"+bairro+"%");
		
		ResultSet result = stm.executeQuery();
		
		List<EnderecoSegurado> lista = new ArrayList<>();
	...
	public void cadastrar(EnderecoSegurado enderecoSegurado) throws ClassNotFoundException, SQLException {
		...
		PreparedStatement stmm = conn.prepareStatement("select * from t_sip_endereco_segurado");
		...
		while (resultGet.next()) {
			EnderecoSegurado enderecoSeguradoGet = parse(resultGet);
			lista.add(enderecoSeguradoGet);
		}
	...
	public void atualizar(EnderecoSegurado enderecoSegurado) throws ClassNotFoundException, SQLException, IdNotFoundException {
		...
		PreparedStatement stm = conn.prepareStatement("update t_sip_endereco_segurado set cep = ?, logradouro = ?, bairro = ?, complemento = ? where id_endereco_segurado = ?");
