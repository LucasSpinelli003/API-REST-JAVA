...
public class ContatoSeguradoDao {
	...
	public List<ContatoSegurado> pesquisarPorEmail(String email) throws SQLException{
		PreparedStatement stm = conn.prepareStatement("select * from t_sip_contato_segurado where email like ?");
	...
	public void cadastrar(ContatoSegurado contatoSegurado) throws ClassNotFoundException, SQLException {
		...
		PreparedStatement stmm = conn.prepareStatement("select * from t_sip_contato_segurado");
		...
		while (resultGet.next()) {
			ContatoSegurado contatoSeguradoGet = parse(resultGet);
			lista.add(contatoSeguradoGet);
		}
		
		int id = lista.size() + 1;

		PreparedStatement stm = conn.prepareStatement(
				"INSERT INTO" + " T_SIP_CONTATO_SEGURADO (id_contato_segurado, telefone,"
						+ " email) " + "values (?, ?, ?)");

		stm.setInt(1, id);
		stm.setString(2, contatoSegurado.getTelefone());
		stm.setString(3, contatoSegurado.getEmail());
		stm.executeUpdate();
	}
	...
	public void atualizar(ContatoSegurado contatoSegurado) throws ClassNotFoundException, SQLException, IdNotFoundException {
		...
		stm.setInt(3, contatoSegurado.getId());
